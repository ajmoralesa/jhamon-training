    ...
    for participant in training_sessions.keys():
        ...
        for tr_session in training_sessions[participant].keys():
            ...
            for ii in np.arange(len(paths)):
                serie = 'set_' + str(ii + 1)
                print('Amos allá con las sesiones de: ' +
                      participant + ' ' + tr_session + ' ' + serie)
...
def _damegauche(data_path, files):
    ...
    filespaths = [data_path + "\\" + str(files[ii])
                  for ii in range(len(files))]
    paths_droite = []
    for ii in range(len(filespaths)):
        file = open(filespaths[ii], "r")
        file.readline()
        info = file.readline()
        file.close()
        if "Gauche" in info:
...
def _segmentaIK(ikpath, participant, tr_session, serie, tidx=0, vidx=1, pidx=2):
    ...
    onsets_1 = _detect_onset(torque, max(torque)*0.25, n_above=80, n_below=0,
                             show=False)[:, 0]
    ...
    for ii in range(len(vonset_precise)):
        ...
        rep['rep_' + str(ii+1)] = {'torque': linear_tnorm(torq)[0],
...
def gravitycorrect(ikpath):
    ...
    data = np.genfromtxt(ikpath, skip_header=100, delimiter="")
    gdata = np.genfromtxt(ikpath, skip_header=79, max_rows=18, delimiter="")
    ...
    if abs(gdat[40]) > 0:
        ...
        # leave out 10 points at each side to avoid extremes
        posk = mypos[15:-15]
        torok = np.flip((gdat[np.argwhere(gdat)]/10)
                        * -1, axis=0)[15:-15].flatten()
