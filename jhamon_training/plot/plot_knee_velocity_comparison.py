...
def plot_knee_velocity_comparison(
    data: pd.DataFrame, output_path: Union[Path, str, None] = None
) -> None:
    """Generate a combined figure with knee velocity comparison plots.

    The figure contains:
    - Top: Knee velocity across training sessions
    ...
    for group_name in ["NH", "IK"]:
        ...
        # Collect data for each training session
        for trses in training_sessions:
            session_data = data[
                (data["tr_group"] == group_name) & (data["trses"] == trses)
            ]
            # Store individual values for scatter plot
            all_velocities.extend(
                [(int(trses.split("_")[1]), v) for v in session_data["knee_v_mean"]]
            )

        # Plot individual points with jitter
        x_coords = [x[0] + x_positions[group_name] for x in all_velocities]
        y_coords = [x[1] for x in all_velocities]
    ...
    ax1.set_ylabel("Angular velocity (°/s)", fontsize=14)
    ...
    pivoted_data = data.pivot_table(
        index=["set", "trses"], columns="tr_group", values="knee_v_mean", aggfunc="mean"
    ).reset_index()
    ...
    mean_knee_v = (pivoted_data["NH"] + pivoted_data["IK"]) / 2
    ...
    ax2.scatter(mean_knee_v, diff, alpha=0.5, color=nh_color)
    ax2.axhline(0, color="red", linestyle="--")
    ax2.axhline(
        mean_diff,
        color=nh_color,
        linestyle="--",
        label=f"Mean difference: {mean_diff:.2f}°/s",
    )
    ax2.axhline(
        upper_limit,
        color=nh_color,
        linestyle="--",
        label=f"Upper limit: {upper_limit:.2f}°/s",
    )
    ax2.axhline(
        lower_limit,
        color=nh_color,
        linestyle="--",
        label=f"Lower limit: {lower_limit:.2f}°/s",
    )

    ax2.set_xlabel("Mean Knee Velocity (°/s)", fontsize=14)
    ax2.set_ylabel("Difference (NH - IK) (°/s)", fontsize=14)
    ...
    ax3.set_xlabel("NH Knee Velocity (°/s)", fontsize=14)
    ax3.set_ylabel("IK Knee Velocity (°/s)", fontsize=14)
